...
;objoff_30	=	initial_y_pos

;objoff_38	= our own address lol

; Sprite_3F1E4:
Obj_S2_AnimalCapsule:
Obj_S2_EggPrison:
Obj3E:
	moveq	#0,d0
	move.b	routine(a0),d0
...
; ----------------------------------------------------------------------------
;y offset, routine, width_pixels, priority (s2 format), mapping_frame
; byte_3F1FE:
Obj3E_ObjLoadData:
	dc.b   0,  2,$20,  4,  0	; 0 ;objoff_38	;main body
	dc.b $28,  4,$10,  5,  4	; 5	;objoff_3A
	dc.b $18,  6,  8,  3,  5	; 10 ;objoff_3C	(!)
	dc.b   0,  8,$20,  4,  0	; 15 ;objoff_3E
	even
; ===========================================================================
;init
loc_3F212:
	movea.l	a0,a1			;get our address in a1
	lea	objoff_38(a0),a3	;load object offset into a3
	lea	Obj3E_ObjLoadData(pc),a2	;Get type data in a2
	moveq	#3,d1			;set loop count?
	bra.s	loc_3F228		;go to object spawn loop, starting by initializing self.
; ===========================================================================
...
loc_3F228:
	move.l	code(a0),code(a1) ; load obj
	move.w	x_pos(a0),x_pos(a1)
	move.w	y_pos(a0),y_pos(a1)
	...
	moveq	#0,d0
	move.b	(a2)+,d0	;get y offset in d0
	sub.w	d0,y_pos(a1)	;subtract from y pos (moves up)
...
loc_3F2B4:
	...
	bne.s	+
	move.l	#Obj_Explosion,code(a1) ; load obj
	addq.b	#2,routine(a1)
...
-	jsr	(AllocateObject).l
	bne.s	+
	move.l	#Obj_Animal,code(a1) ; load obj
	move.w	x_pos(a0),x_pos(a1)
	move.w	y_pos(a0),y_pos(a1)
	add.w	d4,x_pos(a1)
...
-	cmp.l	code(a1),d1	;is this an animal?
	beq.s	+	; rts
